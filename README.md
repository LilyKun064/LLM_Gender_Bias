...
├── code/
│   ├── full_experiment.py         # Stage 1: run full experiment, collect model outputs
│   ├── analysis_pt1.py            # Stage 2: Bayesian pronoun analysis
│   ├── score_human_code_with_LLM.py  # Stage 3: explanation scoring with LLM + rules
│   ├── make_human_coding_file.py  # Utility: sample rows for manual coding
│   └── analysis_pt2.py            # Stage 4: Bayesian reasoning models
├── config/
│   └── .env                       # API keys and secrets (not tracked in git)
├── data/
│   ├── raw/                       # Full experiment outputs from Stage 1
│   │   └── exp_full_compare_models_*.csv
│   ├── human_coding/              # Human coding samples and LLM-scored subsets
│   │   ├── human_coding_sample_200.csv
│   │   ├── human_coding_sample_200.xlsx
│   │   └── human_coding_sample_200_LLM_scored.xlsx
│   └── reasons_full_compare/      # Full reasoning dataset after Stage 3
│       └── exp_full_LLM_scored.xlsx
├── figures/                       # Plots and figures for the manuscript
├── manuscript/                    # LaTeX source for the workshop paper
│   ├── manuscript.tex
│   ├── appendix_I_Body.tex        # Prompt templates (Appendix I)
│   └── appendix_codebook_body.tex # Reasoning codebook (Appendix II)
├── notebooks/
│   └── 01_intro.ipynb             # Exploratory / demo notebook(s)
├── results/
│   ├── bayes_hier_full/           # Outputs from analysis_pt1 (Stage 2)
│   └── analysis_pt2_logitnormal/  # Outputs from analysis_pt2 (Stage 4)
├── src/
│   └── full_experiment.py         # Alternate entry point / mirrored script
├── README.md                      # Project description and pipeline
├── .gitignore
├── prompt.docx                    # Prompt draft / notes
...
1. **Main response**  
2. **Gender inference** (“he/she only”)  
3. **Explanation** (“Why did you pick this pronoun?”)

This creates a rich dataset of **1,680 cases × 4 models = 6,720 explanation events**.
...
# 1️⃣ Stage 1 — Full Experiment Generation

**Script:** `code/full_experiment.py`  
This script:

- Iterates over *all* scenario × tone × factor combinations  
...
## Stage 2 — Pronoun-Level Bayesian Analysis (`analysis_pt1.py`)
...
- `trace_hierarchical.nc` — Full posterior draws for all hierarchical parameters.
- `summary_global_and_sigma.csv` — Posterior summaries for the global intercept and all variance components.
- `summary_model_baseline_p_she.csv` — Baseline P(she) for each model.
- `summary_model_pairwise_diff_p_she.csv` — Pairwise differences between models.
- `summary_scenario_baseline_p_she.csv` — Scenario-level shifts in P(she).
- `summary_tone_baseline_p_she.csv` — Tone-level shifts in P(she).
- `summary_factor_baseline_p_she.csv` — Factor-specific shifts (occupation / food / hobby profile).
- `cell_probs_she_hierarchical.csv` — Posterior P(she) for every observed (scenario × model × tone × factor).
- `cell_model_differences_p_she.csv` — Cell-wise pairwise model contrasts P(she|model2) – P(she|model1).
...
## Stage 3 — Explanation Scoring (`score_human_code_with_LLM.py`)

Stage 3 takes the explanations from Stage 1 (“Why did you choose this pronoun?”) and converts them into **quantitative reasoning codes** using a two-step system:
...
- `response_pronoun` — the model’s gendered description (he/she)
- `response_why` — the explanation to be coded
...
- `fact` — based on scenario content, job duties, concrete details  
- `tone_reason` — based on direct/polite tone cues  
- `style` — based on writing style, structure, or formality  
- `emotion` — based on emotional traits, personality impressions  
...
- `stereotype_gender` ∈ {masculine, feminine, both, none, unclear}
...
- `stereo = 1` → the explanation uses stereotypes aligned with pronoun  
- `avoid_stereo = 1` → stereotypes invoked but pronoun contradicts stereotype direction  
- `other = 1` → no clear stereotype use or ambiguous mapping  
...
## Stage 4 — Bayesian Reasoning Models (analysis_pt2.py)
...
fact*, tone_reason*, style*  ~  MVN( μ + group-level effects , Σ )
...
- Full cell-level compositions (scenario × model × tone × factor)
...
with weakly informative priors and hierarchical standard deviations σ_⋅ ~ HalfNormal.
